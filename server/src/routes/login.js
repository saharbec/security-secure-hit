...
router.post('/login', async (req, res) => {
  ...
  db.query(
    ...
    (error, results, fields) => {
      ...
      if (
        results[0].failedLoginAttempts >= passwordConfig['maximum attempts']
      ) {
        return res
          .status(500)
          .send(
            'your account has been disabled due to too many failed attempts'
          );
      }
      bcrypt.compare(
        password,
        results[0].password,
        (bcryptError, bcryptResults) => {
          if (bcryptError) {
            return res.status(500).send('An error occurred');
          }
          if (!bcryptResults) {
            db.query(
              'UPDATE users SET failedLoginAttempts = failedLoginAttempts+1 WHERE email = (?)',
              [email]
            );
