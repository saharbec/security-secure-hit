    ...
    jwt.verify(
      ...
      (error, authData) => {
        ...
        bcrypt.compare(
          ...
          async (bcryptError, bcryptResults) => {
            if (bcryptError) {
              ...
              async (oldPassError, oldPassResult) => {
                ...
                const oldPasswordsArr =
                  oldPassResult[0].oldPasswords === null
                    ...
                    : [
                        oldPassResult[0].password,
                        ...oldPassResult[0].oldPasswords.split(','),
                      ].slice(0, passwordConfig.history);
                isPrevPass = await checkIfPassExists(
                  newPassword,
                  oldPasswordsArr
                );
                if (isPrevPass) {
                  return res
                    .status(400)
                    .send('You have already used this password');
                }
