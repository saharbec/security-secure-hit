...
export default function Register() {
  ...
  const getpasswordRules = useCallback(() => {
    Axios.get(config.serverUrl + '/passwordRequirements')
      .then((response) => {
        setpasswordRules(response.data);
    ...
    <Container component="main" maxWidth="xs">
      ...
      >
        ...
        <Box component="form" noValidate onSubmit={handleSubmit} sx={{ mt: 3 }}>
          ...
          {passwordRules && isChanged && (
            <Alert
              severity={
                passwordRules['min password length'] <= UserPasswordvalues.password.length ? 'success' : 'error'
              }
              sx={{ mt: 1 }}
            >
              password must contain more than {passwordRules['min password length']} characters
            </Alert>
          )}
          {passwordRules &&
            isChanged &&
            Object.keys(passwordRules.character.settings).reduce((filtered, key) => {
              if (passwordRules.character.settings[key]) {
                let re = new RegExp(passwordRules.character.regex[key]);
                filtered.push(
                  <Alert
                    key={key}
